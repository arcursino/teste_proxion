...
```bash
dk bash  # starts bash inside "proxion" container
...
* `dkup` uses docker-compose to start 3 containers: postgres, nginx, and proxion.
...
* Once `dkup` is running, `dk <command>` will run `<command>` inside the `proxion` container. So `dk bash` will get you "logged in" as root inside that container. Once inside, you need to run Django's `manage.py` commands to initialize the database properly.
* The proxion container runs 3 services:
 * django on port 8000
...
```bash
mkvirtualenv proxion -p python3  # creates a python3 virtualenv
...
Install docker and nginx. Create an empty postgres database proxion owned by a user proxion.

On your remote machine, create a file ~/proxion.env:

```
DJANGO_DB_PASSWORD=<proxion's password>
DJANGO_DB_HOST=<database_ip>
DJANGO_DB_NAME=proxion
DJANGO_DB_USER=proxion
DJANGO_DEBUG=0
```
...
server {
    ...
    location /static {
        alias /home/ubuntu/dkdata/proxion/static;
...
(Replace "proxion.example.com" with your production domain)
