...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development proxion npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop proxion
    docker rm proxion
    docker run --name proxion -d --env-file /home/ubuntu/proxion.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/proxion:/dkdata \
        proxion start_web.sh
}

function deploy_prod {
  rsync -av --exclude frontend/node_modules --exclude frontend/.nuxt * ubuntu@$HOST_PROD:./proxion/
...
echo_green "Welcome to proxion's dev env"
